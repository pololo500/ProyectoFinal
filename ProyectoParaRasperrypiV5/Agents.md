...
4. **Manejo de Silencios:** El micrófono debe grabar en un buffer circular. La grabación de un segmento se corta y se envía a `faster-whisper` cuando ocurre lo primero de: (a) `silero-vad` detecta un silencio continuo de ~1.2 segundos (3.0s si el niño está triste o enojado), o (b) la captura lleva 15 segundos. El tope de 15s evita escuchas interminables por ruido de fondo.
